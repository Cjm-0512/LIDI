...
function Pagination (data) {
    ...
    let ele = data.ele;
    $(ele + '>div').html('');
    let currentPage = data.currentPage ? data.currentPage : 1;
    let callback = data.callback ? data.callback : function(pageNum){
    ...
    function inputPage () {
        ...
        if(($(ele + ' input').val() > 9 && data.totalPage > 9) || (isInit && currentPage > 10)){
            if(currentPage%10 < 5){
                let a = (currentPage - currentPage%10)/10;
                let b = a - 1;
                let c = a + b;
    ...
    if(data.totalCount != 0){
        $(ele + ' .Pagination').append('<span class="numpage">每<input type="text" placeholder="10"/>条</span>'+
            '<span class="prepage"><img src="../images/previous.png" alt=""/></span>' +
            '<div class="list_box clearfix"><ul></ul></div>' +
            '<span class="nextpage"><img src="../images/next.png" alt=""/></span>' );
